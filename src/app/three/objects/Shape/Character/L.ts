...
export class L {
    ...
    constructor(charCount: number, material: THREE.ShaderMaterial) {
        ...
        const geometry1 = this.createCylinderGeometry(charCount, material, -charCount * 0.8, 0, 0, 0);
        const geometry2 = this.createCylinderGeometry(charCount, material, -charCount * 1.0, 0, 0, Math.PI / 2);
    ...
    // 単一のジオメトリ作成メソッド
    private createCylinderGeometry(charCount: number, material: THREE.ShaderMaterial, x: number, y: number, z: number, rotationZ: number): THREE.BufferGeometry {
