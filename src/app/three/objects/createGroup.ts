...
function createGroup(objectType: string, size: number, number: number, x: number, y: number, z: number) {
    ...
    const BoxList: THREE.Mesh[] = [];
    const circlesList: THREE.Mesh[] = [];
    for (let i = 0; i < number; i++) {
        let mesh;
        if (objectType === object_List[0]) {
            const radius = size;
            const circle = new Circle(radius, position)
            ...
            const line = size;
            const box = new Box(line, position)
...
function GroupRotate(group: THREE.Group, rotateTypes: string[], angleStep: number, iterator: number) {
...
function complexcreateTorusOnPath(pathRadius: number, torusRadius: number, tubeRadius: number, numberofTorus: number, numberofRing: number) {
    ...
    for (let i = 0; i < numberofTorus; i++) {
        ...
        for (let j = 0; j < numberofRing; j++) {
            ...
            const torusPosition = new THREE.Vector3(position.x + x, position.y + y, z);
            const boxPosition = new THREE.Vector3(position.x + x, position.y + y, z);

            const { group: Toruses } = createGroup('circle', 5, 10, torusPosition.x, torusPosition.y, torusPosition.z);
            const { group: Boxes } = createGroup('box', 5, 5, boxPosition.x, boxPosition.y, boxPosition.z);
